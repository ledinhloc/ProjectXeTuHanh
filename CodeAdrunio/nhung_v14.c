...
const int baseSpeed = 255;        // Tốc độ cơ bản của động cơ
...
const unsigned long DISTANCE_CHECK_INTERVAL = 500;  // Kiểm tra mỗi 500ms
...
void avoidObstacle() {
  ...
  if (dists[1] > obstacleDistance) {
    ...
    return; //return để chạy theo line
  }
...
void mainChayTheoLine() {
  ...
  if (sensorSum > 0) {
    ...
    if (error > 0) {
      // Bên trái lùi, bên phải tiến
      motorLeftFront.setSpeed(baseSpeed);
      motorLeftRear.setSpeed(baseSpeed);
      motorRightFront.setSpeed(baseSpeed);
      motorRightRear.setSpeed(baseSpeed);
    ...
    // Nếu đi thẳng (error = 0)
      ...
      // Cả hai bên đều tiến với tốc độ thấp hơn để ổn định
      int stableSpeed = baseSpeed * 0.8;
      
      motorLeftFront.setSpeed(stableSpeed);
      motorLeftRear.setSpeed(stableSpeed);
      motorRightFront.setSpeed(stableSpeed);
      motorRightRear.setSpeed(stableSpeed);
    ...
    // Mất vạch: rẽ theo hướng cuối cùng đã lưu với tốc độ thấp hơn
    int turnSpeed = baseSpeed * 0.7;
    
    if (lastTurnDirection == 'L') {
      // Bên trái lùi, bên phải tiến
      motorLeftFront.setSpeed(turnSpeed);
      motorLeftRear.setSpeed(turnSpeed);
      motorRightFront.setSpeed(turnSpeed);
      motorRightRear.setSpeed(turnSpeed);
